<?php
  class Message extends User{
    ...
    public function notification($user_id){
      $stmt = $this->pdo->prepare("SELECT * FROM `notification` N LEFT JOIN `users` U ON N.`notificationFrom` = U.`user_id` 
      ...
      WHERE N.`notificationFor` = :user_id 
      AND N.`notificationFrom` != :user_id GROUP BY `ID`"); /*N.`notificationForm` != :user_id GROUP BY `ID` "); Add after*/
