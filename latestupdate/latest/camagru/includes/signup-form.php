	...
	if(empty($screenName) or empty($password) or empty($email)){
		...
		if(!filter_var($email)) {
			...
			if($getFromU->checkEmail($email) === true){
				...
				$user_id = $getFromU->create('users', array('email' => $email, 'password' => md5($password) , 'screenName' => $screenName, 'profileImage' => 'assets/images/defaultProfileImage.png', 'profileCover' => 'assets/images/defaultCoverImage.png', 'verified' => '0', 'verification_code' => $verificationCode));
				$_SESSION['user_id'] = $user_id;
